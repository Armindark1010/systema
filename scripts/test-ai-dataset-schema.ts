...
{
  ok('the database is at version 4', /version = 4,/.test(database))
  ...
  ok('the chain ends at the declared database version',
    declared[declared.length - 1]?.[1] === 4)

  // v4 must be additive: no data-losing statement in the new migration.
  const m34 = database.slice(
    database.indexOf('MIGRATION_3_4 = object'),
    database.indexOf('private val MIGRATIONS'))
